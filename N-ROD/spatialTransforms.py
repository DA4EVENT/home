...
class Normalize(object):
    ...
    def __init__(self, mean, std, imagenet_norm):
        self.mean = mean
        self.std = std
    ...
    def __call__(self, tensor, rot):
        ...
        if tensor.size(0) != 3:
            if self.imagenet_norm:
                mean = mean * (tensor.size(0)//3)
                std = std * (tensor.size(0)//3)
            else:
                mean = [0.0] * (tensor.size(0))
                std = [1.0] * (tensor.size(0))


        assert tensor.size(0) == len(mean) == len(std)
