...
def get_torch_transforms(transform_compose, param):
    """
    ...
    for tr in transform_compose.transforms:
        ...
        if class_name == "Normalize":
            if param.imagenet_norm:
                args_and_values["mean"] = args_and_values["mean"] * (param.channels_event//3)
                args_and_values["std"] = args_and_values["std"] * (param.channels_event//3)
            else:
                args_and_values["mean"] = [0.0] * (param.channels_event)
                args_and_values["std"] = [1.0] * (param.channels_event)
